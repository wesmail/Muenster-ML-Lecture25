...
def load_events(path, chamber_id=None, start=0, end=-1):
    ...
    evs = evs[
        ["event_id", "hit_id", "x", "z", "a", "chamber_id", "layer_id", "particle_id"]
    ]
        ...
        for h_1 in data:
            for h_2 in data:
                ...
                for h_3 in data:
                    if h_2[0] == h_3[0]:
                        continue
                    if abs(h_2[4] - h_3[4]) != 1 and abs(h_1[4] - h_3[4]) != 2:
                        continue
        ...
        for j in particle_ids:
            ...
            for df1, pid1 in list_df_1:
                for df2, pid2 in list_df_2:
                    ...
                    offset = (
                        24 - data.shape[0]
                    )  # 24 is a perfect track with the correct number of hits
                    if offset < 0:
                        continue
...
def create_seeds(events):
    ...
    for i in tqdm(range(len(events))):
        ...
        for pid in np.unique(event.particle_id):
            ...
            seed = 4
            data = df.iloc[0:seed, :].to_numpy() / np.array([10.0, 100.0, 0.01])
            target = df.iloc[seed : seed * 2, :].to_numpy() / np.array(
                [10.0, 100.0, 0.01]
            )
...
def create_graph_list(events):
    ...
    for idx in tqdm(range(nevts)):
        ...
        feature_names = ["x", "z"]
        feature_scale = np.array([100.0, 100.0])
...
def graph_generator(in_graphs, start=0, end=100):
    ...
    return [np.stack(X_padded), np.stack(Ri_padded), np.stack(Ro_padded)], np.stack(
        y_padded
    )
